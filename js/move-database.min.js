...
Microsoft.WorkloadMonitor/monitors,0,0,0`;function parseMoveCSV(csv){const db={};csv.split('\n').forEach(line=>{const parts=line.split(',');if(parts.length>=4&&parts[0].trim().includes('/')){const rg=parseInt(parts[1],10);const sub=parseInt(parts[2],10);const reg=parseInt(parts[3],10);db[parts[0].trim().toLowerCase()]={moveRG:isNaN(rg)?0:rg,moveSub:isNaN(sub)?0:sub,moveRegion:isNaN(reg)?0:reg};}});return db;}let MOVE_DB=parseMoveCSV(MOVE_DB_RAW);const MOVE_DB_LIVE_URL='https://raw.githubusercontent.com/tfitzmac/resource-capabilities/master/move-support-resources-with-regions.csv';let moveDbSource='local';(async function refreshMoveDB(){try{const res=await fetch(MOVE_DB_LIVE_URL,{cache:'no-cache'});if(!res.ok)throw new Error(res.statusText);const csv=await res.text();const live=parseMoveCSV(csv);if(Object.keys(live).length>100){MOVE_DB=live;moveDbSource='live';if(typeof allResults!=='undefined'&&allResults.length){allResults.forEach(r=>{const info=lookupResourceType(r.type);if(info){r.moveRG=info.moveRG;r.moveSub=info.moveSub;r.moveRegion=info.moveRegion;r.status=getStatus(info);}});updateStats();renderTable();}}}catch{moveDbSource='local';}if(typeof updateDbSourceBadge==='function')updateDbSourceBadge();})();const MOVE_NOTES={'microsoft.sql/servers':'noteSqlServer','microsoft.sql/servers/databases':'noteSqlDb','microsoft.sql/servers/elasticpools':'noteSqlElasticPool','microsoft.sql/servers/jobaccounts':'noteSqlChild','microsoft.sql/servers/jobagents':'noteSqlChild','microsoft.sql/servers/databases/backuplongtermretentionpolicies':'noteSqlChild','microsoft.sql/managedinstances':'noteSqlManagedInst','microsoft.sql/managedinstances/databases':'noteSqlChild','microsoft.compute/virtualmachines':'noteVm','microsoft.compute/disks':'noteDisk','microsoft.compute/virtualmachines/extensions':'noteVmExt','microsoft.compute/availabilitysets':'noteAvailSet','microsoft.compute/snapshots':'noteSnapshot','microsoft.compute/virtualmachinescalesets':'noteVmss','microsoft.network/networkinterfaces':'noteNic','microsoft.network/publicipaddresses':'notePublicIp','microsoft.network/loadbalancers':'noteLoadBalancer','microsoft.network/networksecuritygroups':'noteNsg','microsoft.network/virtualnetworks':'noteVnet','microsoft.network/applicationgateways':'noteAppGw','microsoft.web/sites':'noteAppService','microsoft.web/serverfarms':'noteAppPlan','microsoft.web/certificates':'noteCert','microsoft.certificateregistration/certificateorders':'noteAppService','microsoft.domainregistration/domains':'noteAppService','microsoft.documentdb/databaseaccounts':'noteCosmosDb','microsoft.dbformysql/servers':'noteDbReplica','microsoft.dbforpostgresql/servers':'noteDbReplica','microsoft.dbformariadb/servers':'noteDbReplica','microsoft.containerregistry/registries':'noteContainerReg','microsoft.hdinsight/clusters':'noteHdInsight','microsoft.operationalinsights/workspaces':'noteLogAnalytics','microsoft.automation/automationaccounts':'noteAutomation','microsoft.automation/automationaccounts/runbooks':'noteAutomationRunbook','microsoft.automation/automationaccounts/configurations':'noteAutomationChild','microsoft.insights/components':'noteAppInsights','microsoft.cache/redis':'noteRedis','microsoft.keyvault/vaults':'noteKeyVault','microsoft.eventgrid/eventsubscriptions':'noteEventGridSub','microsoft.apimanagement/service':'noteApiMgmt','microsoft.recoveryservices/vaults':'noteRecovery','microsoft.streamanalytics/streamingjobs':'noteStreamAnalytics',};