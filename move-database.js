/* ==========================================================
   ...
   Source: https://github.com/tfitzmac/resource-capabilities
...
Microsoft.Attestation/attestationproviders,1,1,0
...
Microsoft.CognitiveServices/CognitiveSearch,1,1,0
...
Microsoft.DataProtection/backupvaults,0,0,0
Microsoft.DataShare/accounts,1,1,0
Microsoft.DBforMariaDB/servers,1,1,0
Microsoft.DBforMySQL/flexibleServers,1,1,0
Microsoft.DBforMySQL/servers,1,1,0
...
Microsoft.DBforPostgreSQL/serversv2,1,1,0
...
Microsoft.Maintenance/maintenanceconfigurations,1,1,1
...
Microsoft.Network/frontdoors,0,0,0
...
Microsoft.Network/vpngateways,0,0,0
...
Microsoft.SqlVirtualMachine/sqlvirtualmachinegroups,1,1,0
...
// Live data source URL (public GitHub — same source that feeds Microsoft docs)
const MOVE_DB_LIVE_URL =
  'https://raw.githubusercontent.com/tfitzmac/resource-capabilities/master/move-support-resources-with-regions.csv';

// Track data source status
let moveDbSource = 'local'; // 'local' | 'live'

// Try fetching the latest data from GitHub on page load
(async function refreshMoveDB() {
  try {
    const res = await fetch(MOVE_DB_LIVE_URL, { cache: 'no-cache' });
    if (!res.ok) throw new Error(res.statusText);
    const csv = await res.text();
    const live = parseMoveCSV(csv);
    // Only replace if we actually got data
